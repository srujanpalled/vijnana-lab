  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 flex flex-col items-center justify-center px-4 py-12 relative overflow-hidden">
      ...
      <div className="text-center mb-10 relative z-10">
        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 mb-5">
          <Sparkles size={14} className="text-yellow-400" />
          <span className="text-xs font-bold text-gray-700 dark:text-gray-700 dark:text-gray-300 uppercase tracking-widest">Choose Your Syllabus</span>
        </div>
        <h1 className="text-4xl md:text-5xl font-black text-slate-900 dark:text-slate-900 dark:text-white mb-3">
        ...
        <div className={`grid grid-cols-1 md:grid-cols-3 gap-5 max-w-4xl w-full relative z-10 transition-all duration-500 ${animating ? 'opacity-0 scale-95' : 'opacity-100 scale-100'}`}>
          {BOARDS.map((board) => (
            <button
              ...
              className="group relative p-6 rounded-2xl border border-black/10 dark:border-white/10 bg-black/5 dark:bg-white/5 backdrop-blur-xl transition-all duration-300 hover:scale-105 hover:border-opacity-50 text-left overflow-hidden active:scale-95"
            ...
            >
              ...
              <div className="relative z-10">
                ...
                <h3 className="text-xl font-black text-slate-900 dark:text-slate-900 dark:text-white mb-1">{board.name}</h3>
                <p className="text-sm text-gray-600 dark:text-gray-400 mb-3">{board.fullName}</p>
                <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider" style={{ color: board.color }}>
                  <School size={12} />
                  {board.states}
                </div>
                <div className="mt-4 flex items-center gap-1 text-white/60 group-hover:text-slate-900 dark:text-slate-900 dark:text-white transition-colors text-sm font-bold">
      ...
      {phase === 'standard' && (
        <div className={`max-w-2xl w-full relative z-10 transition-all duration-500 ${animating ? 'opacity-0 scale-95' : 'opacity-100 scale-100'}`}>
          {/* Back button */}
          <button onClick={goBack} className="mb-5 flex items-center gap-2 text-gray-600 dark:text-gray-400 hover:text-slate-900 dark:text-slate-900 dark:text-white transition-colors text-sm font-bold">
            ← Change Board
          </button>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            {STANDARDS.map((std) => (
              <button
                ...
                className="group relative p-8 rounded-2xl border border-black/10 dark:border-white/10 bg-black/5 dark:bg-white/5 backdrop-blur-xl transition-all duration-300 hover:scale-105 text-left overflow-hidden active:scale-95"
              >
                <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 rounded-2xl"
                  style={{ boxShadow: `inset 0 0 40px ${std.color}15, 0 0 30px ${std.color}10` }} />
                <div className="relative z-10">
                  <div className="text-5xl mb-4">{std.icon}</div>
                  <h3 className="text-xl font-black text-slate-900 dark:text-slate-900 dark:text-white mb-1">{std.name}</h3>
                  <p className="text-sm text-gray-600 dark:text-gray-400 mb-3">{std.subtitle}</p>
      ...
      {selectedBoard && selectedStandard && phase === 'board' && (
        <div className="mt-8 relative z-10 text-center">
          ...
          <div className="inline-flex items-center gap-3 px-5 py-2.5 rounded-xl bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10">
            <span className="text-slate-900 dark:text-slate-900 dark:text-white font-bold text-sm">{selectedBoard} · {selectedStandard}</span>
