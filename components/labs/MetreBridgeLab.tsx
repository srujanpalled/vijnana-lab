...
const MetreBridgeScene = ({ jockeyX, R_known, isNearBalance, galvDeflection, X_calc }: { jockeyX: number, R_known: number, isNearBalance: boolean, galvDeflection: number, X_calc: number }) => {
  ...
  // Galvanometer needle and jockey references
  const needleRef = React.useRef<THREE.Group>(null);
  const jockeyRef = React.useRef<THREE.Group>(null);
  const wireLineRef = React.useRef<any>(null);
  ...
  useFrame(() => {
    ...
    if (jockeyRef.current) {
        // Smooth jockey transition
        const targetX = wireX0 + jockeyX * wireLength;
        jockeyRef.current.position.x = THREE.MathUtils.lerp(jockeyRef.current.position.x, targetX, 0.2);
        
        if (wireLineRef.current) {
            const jx = jockeyRef.current.position.x;
            wireLineRef.current.geometry.setPositions([
                jx, 0.4, 1,
                jx, 0.8, 0,
                0, 0.2, -0.8
            ]);
    ...
    <group position={[0, -0.5, 0]}>
      ...
      <RoundedBox args={[1.2, 0.8, 1]} position={[-2, 0.4, -0.8]} radius={0.05} castShadow>
        ...
        <Html position={[0, 0.5, 0]} center>
          <div className="bg-black/80 px-2 py-0.5 rounded text-[10px] font-mono text-blue-600 dark:text-blue-300 border border-blue-500/50 backdrop-blur-md shadow-[0_0_10px_rgba(59,130,246,0.3)]">
            R = {R_known}Ω
          </div>
        </Html>
      </RoundedBox>
      {/* Connecting Wires for R (Bioluminescent Glow) */}
      <Line points={[[-2, 0.4, -0.5], [-2, 0.05, -0.1]]} color="#00ff88" lineWidth={3} />
      <Line points={[[-2.5, 0.4, -0.5], [-3.8, 0.05, 0]]} color="#00ff88" lineWidth={3} />
      ...
      <Cylinder args={[0.15, 0.15, 0.8, 16]} position={[2, 0.2, -0.8]} rotation={[0, 0, Math.PI/2]} castShadow>
        ...
        <Html position={[0, 0.4, 0]} center>
          <div className="bg-black/80 px-2 py-0.5 rounded text-[10px] font-mono text-emerald-300 border border-emerald-500/50 backdrop-blur-md shadow-[0_0_10px_rgba(16,185,129,0.3)]">
            X ≈ {X_calc.toFixed(1)}Ω
          </div>
        </Html>
      </Cylinder>
      {/* Connecting Wires for X (Bioluminescent Glow) */}
      <Line points={[[1.7, 0.2, -0.8], [0.5, 0.05, -0.1]]} color="#00ff88" lineWidth={3} />
      <Line points={[[2.3, 0.2, -0.8], [3.8, 0.05, 0]]} color="#00ff88" lineWidth={3} />

      {/* Galvanometer (Glassmorphic Casing) */}
        ...
        <group position={[0, 0, 0.22]}>
            ...
            <group ref={needleRef} position={[0, -0.1, 0]}>
                <mesh position={[0, 0.15, 0]}>
                    ...
                    <meshBasicMaterial color={isNearBalance ? "#00ff88" : "#ef4444"} />
                </mesh>
            </group>
        </group>
      </group>
      {/* Wire from Middle strip to Galvanometer (Bioluminescent) */}
      <Line points={[[0, 0.05, -0.1], [0, 0.1, -0.8]]} color="#00ff88" lineWidth={3} />

      {/* Jockey and its wire */}
      <group ref={jockeyRef} position={[wireX0 + jockeyX*wireLength, 0.4, 1]}>
        ...
        <Html position={[0, -0.5, 0]} center>
            <div className="bg-black/60 px-1.5 py-0.5 rounded text-[10px] font-mono text-amber-600 dark:text-amber-300 border border-amber-500/30 backdrop-blur-md">
            l = {(jockeyX * 100).toFixed(1)}cm
            </div>
        </Html>
      </group>
      
      {/* Jockey Wire leading to Galvanometer (Bioluminescent) */}
      <Line ref={wireLineRef} points={[[wireX0 + jockeyX*wireLength, 0.4, 1], [wireX0 + jockeyX*wireLength, 0.8, 0], [0, 0.2, -0.8]]} color="#00ff88" lineWidth={3} />
    ...
    <div className="flex flex-col md:flex-row h-full w-full bg-slate-950">
      <div className="flex-1 relative rounded-2xl overflow-hidden m-4 border border-black/10 dark:border-white/10 shadow-2xl">
        ...
        <div className="absolute top-4 left-4 bg-black/60 backdrop-blur-md border border-black/10 dark:border-white/10 rounded-xl px-4 py-3 shadow-xl max-w-xs">
          ...
          <p className="text-slate-900 dark:text-slate-900 dark:text-white font-bold text-sm">3D Metre Bridge</p>
          <p className="text-xs text-slate-600 dark:text-slate-400 mt-1">Determine unknown resistance using Wheatstone bridge principle.</p>
        </div>
      </div>
        ...
        <div className="flex-1 p-5 space-y-5 overflow-y-auto">
          ...
          <div className="grid grid-cols-2 gap-2">
            {[
              ...
              <div key={m.label} className="bg-slate-950/50 border border-black/5 dark:border-white/5 rounded-xl p-3 text-center shadow-sm">
          ...
          <div className="bg-slate-950 p-4 rounded-xl border border-black/5 dark:border-white/5 shadow-inner text-xs space-y-2">
            <p className="text-slate-600 dark:text-slate-400 font-bold text-[10px] uppercase tracking-widest mb-2 border-b border-black/5 dark:border-white/5 pb-1">Wheatstone Formula</p>
          ...
          <div className="flex gap-2 pt-2">
            <button onClick={logReading} disabled={!isNearBalance}
              className="flex-1 py-3 rounded-xl text-sm font-bold text-slate-900 dark:text-slate-900 dark:text-white transition-all active:scale-95 disabled:opacity-40 disabled:scale-100 shadow-lg shadow-blue-500/20"
            ...
            <div className="overflow-x-auto text-xs bg-slate-950 rounded-xl border border-black/5 dark:border-white/5 shadow-inner">
              <table className="w-full border-collapse">
                <thead><tr className="bg-slate-900/80 border-b border-black/10 dark:border-white/10">
                  {['#', 'l(cm)', 'R(Ω)', 'X(Ω)', ''].map(h => <th key={h} className="px-3 py-2 text-slate-600 dark:text-slate-400 text-left font-semibold">{h}</th>)}
                </tr></thead>
