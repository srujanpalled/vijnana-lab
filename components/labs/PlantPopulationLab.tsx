...
const PlantPopulationLab: React.FC<Props> = ({ hex }) => {
  ...
  return (
    <div className="flex flex-col md:flex-row h-full w-full bg-[#050505] overflow-hidden text-slate-800 dark:text-slate-200 select-none">
      ...
      {/* 3D Visualization */}
      <div className="flex-1 flex flex-col relative rounded-2xl overflow-hidden m-4 border border-black/10 dark:border-white/10 shadow-[0_0_50px_rgba(34,197,94,0.05)]">
        <div className="absolute inset-x-0 top-0 p-4 bg-gradient-to-b from-black/80 to-transparent z-10 flex justify-between items-start pointer-events-none">
            <div>
                ...
                <p className="text-[11px] font-medium text-slate-600 dark:text-slate-400 uppercase tracking-widest mt-1">3D Terrain Population Density Analysis</p>
            </div>
            <div className="bg-black/40 backdrop-blur-md px-3 py-1.5 rounded-xl border border-black/10 dark:border-white/10 text-[10px] font-bold uppercase tracking-widest shadow-inner text-amber-400 flex items-center gap-2">
        ...
        <div className="absolute bottom-6 left-1/2 -translate-x-1/2 bg-black/60 backdrop-blur-xl p-3 px-6 rounded-full border border-black/10 dark:border-white/10 flex items-center gap-6 shadow-2xl pointer-events-none">
             <div className="text-center">
                 <p className="text-[9px] uppercase font-bold text-slate-600 dark:text-slate-400 tracking-widest mb-0.5">Quadrat Area</p>
                 <p className="font-mono text-sm font-bold text-amber-400">{quadratSize * quadratSize} m²</p>
             </div>
             <div className="w-px h-6 bg-black/10 dark:bg-white/10" />
             <div className="text-center">
                 <p className="text-[9px] uppercase font-bold text-slate-600 dark:text-slate-400 tracking-widest mb-0.5">Live Count</p>
                 <p className="font-mono text-lg font-bold text-slate-900 dark:text-slate-900 dark:text-white leading-none">{currentCount}</p>
             </div>
        </div>
      </div>
      ...
      <div className="w-full md:w-[320px] shrink-0 bg-[#0a0a0a] border-l border-black/10 dark:border-white/10 flex flex-col z-20 shadow-[-10px_0_30px_rgba(0,0,0,0.5)]">
        <div className="p-6 border-b border-black/5 dark:border-white/5">
          ...
          <h2 className="text-xl font-bold text-slate-900 dark:text-slate-900 dark:text-white tracking-tight">Plant Population</h2>
        </div>
        ...
        <div className="flex-1 p-6 space-y-6 overflow-y-auto">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-[#111] border border-black/5 dark:border-white/5 shadow-inner rounded-xl p-4 flex flex-col items-center justify-center">
          ...
          <div className="space-y-3">
            <div className="flex justify-between items-center text-[10px] font-bold uppercase tracking-widest text-slate-600 dark:text-slate-400">
          ...
          {samples.length > 0 && (
            <div className="bg-[#111] border border-black/5 dark:border-white/5 rounded-xl p-4 shadow-inner">
              <div className="flex justify-between items-center mb-3 border-b border-black/10 dark:border-white/10 pb-2">
                 <p className="text-[10px] text-slate-600 dark:text-slate-400 uppercase font-bold tracking-widest">Data Registration</p>
                 <span className="text-[10px] font-mono text-slate-500">{samples.length} logs</span>
              </div>
              <div className="space-y-2 max-h-32 overflow-y-auto pr-2 custom-scrollbar">
                {samples.map((s, i) => (
                  <div key={i} className="flex justify-between items-center text-xs p-2 rounded bg-black/40 border border-black/5 dark:border-white/5">
                    <span className="text-slate-600 dark:text-slate-400 font-bold">Sample {i + 1}</span>
              ...
              <div className="mt-3 pt-3 border-t border-black/10 dark:border-white/10 flex justify-between items-center text-xs">
                <span className="text-slate-700 dark:text-slate-700 dark:text-slate-300 font-bold text-[10px] uppercase tracking-widest">Aggregated Density</span>
